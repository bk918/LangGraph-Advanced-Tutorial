...
def get_weather(query: WeatherQuery) -> dict[str, Any]:
    ...
    """
    logger.info(f"Tool 호출: get_weather(city={query.city}, units={query.units})")
    ...
    logger.info(f"날씨 조회 완료: {query.city} - {weather_data['temperature']}{weather_data['unit']}")
...
def calculate(request: CalculationRequest) -> dict[str, Any]:
    ...
    Note:
        보안상 eval() 대신 제한된 연산자만 허용합니다.
        실제 환경에서는 더 안전한 파서를 사용하세요.
    """
    logger.info(f"Tool 호출: calculate(expression={request.expression})")

    try:
        # 보안: 간단한 수식만 허용 (실제로는 ast.literal_eval 등 사용 권장)
        allowed_chars = set("0123456789+-*/().[] ")
        if not all(c in allowed_chars for c in request.expression):
            return {
                "success": False,
                "error": "허용되지 않는 문자가 포함되어 있습니다. 숫자와 기본 연산자만 사용하세요.",
            }

        result = eval(request.expression)  # 실제 환경에서는 safer 대안 사용
...
def write_note(content: str, filename: str = "note.txt") -> dict[str, str]:
    ...
    """
    logger.info(f"Tool 호출: write_note(filename={filename}, content_length={len(content)})")
    ...
    try:
        ...
        logger.info(f"노트 저장 완료: {filepath}")
        ...
        logger.error(f"노트 저장 오류: {e}")
        return {
            "success": False,
            "error": f"노트 저장 중 오류 발생: {str(e)}",
        }
...
def get_note_content(filename: str) -> str:
    ...
    """
    logger.info(f"Resource 요청: file://notes/{filename}")
    ...
    try:
        ...
        logger.info(f"노트 읽기 완료: {filepath} ({len(content)} bytes)")

        return content

    except Exception as e:
        logger.error(f"노트 읽기 오류: {e}")
        return f"Error: 노트 읽기 중 오류 발생: {str(e)}"
...
def code_review(code: str, language: str = "python") -> list[dict[str, str]]:
    ...
    Returns:
        프롬프트 메시지 리스트
    """
    logger.info(f"Prompt 요청: code_review(language={language}, code_length={len(code)})")
...
def bug_analysis(
    ...
    Returns:
        프롬프트 메시지 리스트
    """
    logger.info(f"Prompt 요청: bug_analysis(language={language})")
...
def print_server_info():
    """서버 정보 출력"""
    logger.info("=" * 70)
    logger.info(f"MCP Server: {mcp.name} v{mcp.version}")
    logger.info(f"Description: {mcp.description}")
    logger.info("=" * 70)
    logger.info(f"Tools: {len(mcp._tools)}")
    for tool_name in mcp._tools.keys():
        logger.info(f"  - {tool_name}")
    logger.info(f"Resources: {len(mcp._resources)}")
    for resource_uri in mcp._resources.keys():
        logger.info(f"  - {resource_uri}")
    logger.info(f"Prompts: {len(mcp._prompts)}")
    for prompt_name in mcp._prompts.keys():
...
async def test_server():
    ...
    logger.info(f"날씨 조회 결과: {json.dumps(weather_result, ensure_ascii=False, indent=2)}\n")

    # 계산
    calc_result = calculate(CalculationRequest(expression="2 + 3 * 4"))
    logger.info(f"계산 결과: {json.dumps(calc_result, ensure_ascii=False, indent=2)}\n")
    ...
    logger.info(f"노트 저장 결과: {json.dumps(note_result, ensure_ascii=False, indent=2)}\n")
    ...
    logger.info(f"서버 설정: {json.dumps(server_config, ensure_ascii=False, indent=2)}\n")
