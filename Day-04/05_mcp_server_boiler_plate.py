...
def github_create_issue(request: CreateIssueRequest) -> dict[str, Any]:
    ...
    """
    logger.info(f"Tool 호출: github_create_issue(repo={request.repo})")
    ...
    logger.info(f"이슈 생성 완료: #{issue_data['number']}")
...
def github_search_issues(request: SearchIssuesRequest) -> dict[str, Any]:
    ...
    """
    logger.info(f"Tool 호출: github_search_issues(repo={request.repo}, query={request.query})")
...
class SendMessageRequest(BaseModel):
    ...
    channel: str = Field(..., description="채널명 (#general), 사용자명 (@john), 또는 채널 ID")
...
def slack_send_message(request: SendMessageRequest) -> dict[str, Any]:
    ...
    """
    logger.info(f"Tool 호출: slack_send_message(channel={request.channel})")
    ...
    if request.channel.startswith("#"):
        ...
        logger.info(f"채널 이름 → ID 변환: {request.channel} → {channel_id}")
        ...
        channel_type = "user"
        logger.info(f"사용자명 → ID 변환: {request.channel} → {channel_id}")
...
class DeletePRReviewRequest(BaseModel):
    ...
    review_author: str | None = Field(None, description="리뷰 작성자 (기본값: 인증된 사용자)")
...
def github_delete_pr_review(request: DeletePRReviewRequest) -> dict[str, Any]:
    ...
    """
    logger.info(f"Tool 호출: github_delete_pr_review(repo={request.repo}, pr={request.pr_number})")
...
def find_document(request: FindDocumentRequest) -> dict[str, Any]:
    ...
    Returns:
        검색 결과 또는 선택 옵션
    """
    logger.info(f"Tool 호출: find_document(title={request.title})")
...
def notify_multiple_users(request: NotifyMultipleRequest) -> dict[str, Any]:
    ...
    Returns:
        전송 결과 (부분 성공 포함)
    """
    logger.info(f"Tool 호출: notify_multiple_users(emails={len(request.emails)}개)")
    ...
    for email in request.emails:
        ...
        if "@" not in email or "." not in email:
            ...
            successful.append({"email": email, "status": "sent", "timestamp": datetime.now().isoformat()})
...
def main():
    ...
    logger.info(f"MCP 서버 시작 중... (Transport: {args.transport})\n")
