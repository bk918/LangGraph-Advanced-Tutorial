...
async def authenticate_user(authorization: str | None) -> Auth.types.MinimalUserDict:
    ...
    if not authorization:
        raise Auth.exceptions.HTTPException(
            status_code=401, detail="인가 헤더가 없습니다"
        )
...
async def on_thread_create(
    ...
    if user_role not in ['admin', 'user']:
        raise Auth.exceptions.HTTPException(
            status_code=403, detail="스레드 생성 권한이 없습니다"
        )
...
async def on_thread_access(
    ...
    # 알 수 없는 역할은 접근 거부
    raise Auth.exceptions.HTTPException(
        status_code=403, detail="스레드 접근 권한이 없습니다"
    )
...
async def on_assistants_access(
    ...
    # 알 수 없는 역할은 접근 거부
    raise Auth.exceptions.HTTPException(
        status_code=403, detail="어시스턴트 접근 권한이 없습니다"
    )
...
async def authorize_store(ctx: Auth.types.AuthContext, value: dict):
    ...
    # API 사용자는 특정 네임스페이스만 접근 가능
        ...
        # 알 수 없는 역할은 접근 거부
        raise Auth.exceptions.HTTPException(
            status_code=403, detail="저장소 접근 권한이 없습니다"
        )