"""
SerenaMCP 프로젝트 관리 시스템

이 파일은 SerenaMCP의 프로젝트 관리 기능을 구현합니다.
프로젝트 설정, 파일 시스템 관리, 언어 서버 생성 등의 기능을 제공합니다.

주요 기능:
- 프로젝트 설정 및 구성 관리
- 파일 시스템 탐색 및 필터링
- .gitignore 기반 무시 파일 처리
- 언어 서버 인스턴스 생성 및 관리
- 프로젝트별 데이터 폴더 관리
- 검색 및 파일 조작 기능

아키텍처:
- ProjectConfig를 통한 설정 기반 관리
- GitignoreParser를 활용한 파일 필터링
- SolidLanguageServer 인스턴스 팩토리
- pathspec을 통한 패턴 매칭

주요 컴포넌트:
- Project: 프로젝트 관리의 핵심 클래스
- 언어 서버 통합
- 파일 시스템 유틸리티
"""
...
class Project:
    """
    SerenaMCP 프로젝트 관리 클래스

    특정 프로젝트 디렉토리와 관련된 모든 설정, 파일 시스템 관리,
    언어 서버 인스턴스 등을 관리하는 핵심 클래스입니다.

    주요 책임:
    - 프로젝트 설정 및 구성 관리
    - 파일 시스템 탐색 및 필터링
    - 무시 파일 패턴 처리 (.gitignore 기반)
    - 언어 서버 인스턴스 생성
    - 프로젝트별 데이터 관리
    - 텍스트 검색 기능 제공
    ...
    Attributes:
        ...
        project_config (ProjectConfig): 프로젝트 설정 객체
        is_newly_created (bool): 새로 생성된 프로젝트인지 여부
        _ignored_patterns (list[str]): 무시할 파일 패턴들
        _pathspec (pathspec.PathSpec): 컴파일된 패턴 매처

    초기화 과정:
    1. 프로젝트 루트 및 설정 저장
    2. Serena 데이터 폴더의 .gitignore 파일 생성
    3. 프로젝트 설정과 .gitignore 파일로부터 무시 패턴 수집
    4. pathspec 매처 설정
    """
    ...
    def __init__(self, project_root: str, project_config: ProjectConfig, is_newly_created: bool = False):
        ...
        주어진 프로젝트 루트와 설정으로 새로운 프로젝트 인스턴스를 생성하고,
        필요한 초기화 작업을 수행합니다.

        Args:
            project_root (str): 프로젝트 루트 디렉토리의 절대 경로
            project_config (ProjectConfig): 프로젝트 설정 객체
            is_newly_created (bool): 프로젝트가 새로 생성되었는지 여부

        초기화 단계:
        1. 기본 속성 설정
        2. Serena 데이터 폴더의 .gitignore 파일 생성
        3. 무시 패턴 수집 (프로젝트 설정 + .gitignore)
        4. pathspec 매처 설정

        Note:
            프로젝트가 새로 생성된 경우 특별한 초기화 로직이
            실행될 수 있습니다.
        """
        ...
        # Serena 데이터 폴더에 .gitignore 파일이 없으면 생성
