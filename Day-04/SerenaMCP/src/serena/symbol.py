"""
SerenaMCP 심볼 분석 및 처리 시스템

이 파일은 SerenaMCP의 심볼 분석, 검색, 편집 기능을 구현합니다.
언어 서버를 통한 정확한 심볼 정보 처리와 코드 편집 기능을 제공합니다.

주요 컴포넌트:
- LanguageServerSymbolLocation: 심볼 위치 정보
- PositionInFile: 파일 내 위치 정보
- Symbol: 심볼 추상 베이스 클래스
- LanguageServerSymbol: LSP 기반 심볼 구현
- ReferenceInLanguageServerSymbol: 심볼 참조 정보
- LanguageServerSymbolRetriever: 심볼 검색 및 조회
- JetBrainsSymbol: JetBrains 기반 심볼 (JetBrains IDE 연동용)
...
아키텍처:
- LSP(Language Server Protocol) 기반 정확한 심볼 분석
- 추상화된 심볼 인터페이스 제공
- 언어 서버와 JetBrains 플러그인 간 통합
...
class LanguageServerSymbolLocation:
    """
    언어 서버 기반 심볼 위치 정보를 담는 데이터 클래스

    Serena 시스템 내에서 심볼을 유일하게 식별하는 심볼 식별자의 (시작) 위치를 나타냅니다.
    파일 경로, 라인 번호, 컬럼 번호로 구성되어 정확한 코드 위치를 지정합니다.
    ...
    Attributes:
        relative_path (str | None): 심볼이 포함된 파일의 상대 경로.
            None인 경우 프로젝트 범위 외부에서 정의된 심볼
        line (int | None): 심볼 식별자가 정의된 라인 번호 (함수, 클래스 등).
            일부 심볼 타입에서는 None일 수 있음 (예: SymbolKind.File)
        column (int | None): 심볼 식별자가 정의된 컬럼 번호.
            일부 심볼 타입에서는 None일 수 있음

    Note:
        위치 정보는 0-based 인덱싱을 사용합니다.
        relative_path는 운영체제에 따라 경로 구분자가 자동으로 변환됩니다.
    """
    ...
    프로젝트 루트를 기준으로 한 파일의 상대 경로입니다.
    None인 경우 프로젝트 범위 외부에서 정의된 심볼을 의미합니다.
    """
    line: int | None
    """
    심볼 식별자가 정의된 라인 번호

    함수, 클래스 등의 심볼 식별자가 정의된 라인 번호입니다.
    0-based 인덱싱을 사용하며, 일부 심볼 타입에서는 None일 수 있습니다.
    (예: SymbolKind.File)
    """
    column: int | None
    """
    ...
    심볼 식별자가 정의된 컬럼 번호입니다. 0-based 인덱싱을 사용하며,
    일부 심볼 타입에서는 None일 수 있습니다. (예: SymbolKind.File)
    """
    ...
    def __post_init__(self) -> None:
        ...
        relative_path가 None이 아닌 경우, 경로 구분자를
        현재 운영체제 표준으로 변환합니다.

        Note:
            - Unix/Linux: '/' 사용
            - Windows: '\\' 사용
            - 이 처리는 LSP 통신에서 일관성을 유지하기 위해 필요합니다.
        """
